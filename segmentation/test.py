...
def main():
    ...
    # Load Model
    model = MultiTaskModel(encoder_name="clip",freeze_encoder=True)
    ...
    with torch.no_grad():
        ...
        for batch in progress:
            ...
            # Save Labels
            gender_true.extend(gender_labels.numpy().tolist())
            gender_pred.extend(gender_prediction.numpy().tolist())
            age_true.extend(age_labels.numpy().tolist())
            age_pred.extend(age_prediction.numpy().tolist())
            race_true.extend(race_labels.numpy().tolist())
            race_pred.extend(race_prediction.numpy().tolist())

            # Save Prediction CSV
            batch_size = len(age_labels)

            gender_prob = outputs["gender_probability"].cpu().view(-1)
            age_prob = outputs["age_probability"].cpu()
            race_prob = outputs["race_probability"].cpu()
    ...
    prediction_df.to_csv("test_result/predictions.csv",index=False)

    # Save Confusion Matrices
    pd.DataFrame(gender_cm).to_csv(
        "test_result/confusion_matrix_gender.csv",
        index=False
    )

    pd.DataFrame(age_cm).to_csv(
        "test_result/confusion_matrix_age.csv",
        index=False
    )

    pd.DataFrame(race_cm).to_csv(
        "test_result/confusion_matrix_race.csv",
        index=False
    )
    ...
    summary.to_csv("test_result/test_summary.csv",index=False)
